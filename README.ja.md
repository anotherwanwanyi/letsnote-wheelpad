...
| `scroll.sensitivity` | `0` | -4..+2 | 倍率テーブル `[5, 7, 10, 14, 20, 28, 40]` のインデックス。`-4` と `-3` は Linux 向けの超低感度拡張。 |
...
スクロール感度がおかしいときは、設定ファイルの `scroll.sensitivity`（-4..+2）で調整してください。本デーモンは自動キャリブレーションを行いません — 履歴容量は Windows 完全互換のため 20 スロット固定です（DECISIONS.md D-021-followup 参照）。

### 高分解能スクロール

円形ジェスチャの確定後は、有効な曲率更新ごとに `REL_WHEEL_HI_RES` または `REL_HWHEEL_HI_RES` の分数 v120 デルタを生成します。120 単位がホイール 1 ノッチに相当します。1 単位未満の丸め残差を保持し、移動デッドバンドを 8 デバイス単位としているため、指を止めてから同じ接触で再開した場合も速やかに反応します。古いアプリケーションとの互換性のため、従来の累算器境界では `REL_WHEEL` / `REL_HWHEEL` の整数ノッチも別途生成します。仕様に従うコンシューマーは高分解能ストリームか従来ストリームの一方を選び、両方を加算しません。この処理は操作中のスクロールを滑らかにしますが、慣性スクロールは追加しないため、指が静止すればイベントも停止します。ポインター入力との区別に必要な最初の 7.5° の意図判定遅延は残ります。
