...
- 円形スクロールの候補中に 2 本目の指が触れると、デーモンは透過専用の `MultiTouch` 状態に入ります。物理イベントはすべて libinput に転送され、全ての指が離れるまで円形認識は無効のままです。2 本目だけを離しても、同じジェスチャの途中で円形スクロールが再び有効になることはありません。
- 中心角 15° の開始しきい値を超えて円形スクロールが確定した後は、全ての指が離れるまで円形スクロールがその入力ストリームを所有します。確定後に追加された指は libinput へ公開されず、検出器は最初の tracking ID だけを追跡します。最初の指が先に離れても、別の指を円形軌跡へ継ぎ足すことはありません。
- 終了時には、捕捉したマルチタッチスロットを明示的に解放し、パッドが対応する全ての接触サマリーキーを 0 に戻します。円形スクロール中に抑止された指離しや指本数の変化が libinput に古い接触や「ゴーストフィンガー」を残すことを防ぎます。

要するに、円形スクロールの確定前はマルチフィンガー入力が優先され、確定済みの円形ジェスチャは全指リリースまで所有権を維持します。次の接触シーケンスでは改めて判定を開始します。

## 仕組み（一段落版）

起動時に物理タッチパッドを `EVIOCGRAB` で恒久的に占有し、libinput がアタッチする 2 つの仮想 `uinput` デバイス（物理パッド能力をミラーリングしたタッチパッドと、ホイール）を生成します。円形ジェスチャが確定していない間は、物理タッチイベントを仮想タッチパッドへそのまま転送するため、カーソル・タップ・クリック・マルチフィンガージェスチャは従来どおり動作します。6 状態の FSM（`Idle`、`Contact`、`Moving`、`MultiTouch`、`Scrolling`、`Debounce`）は確定前のマルチフィンガー入力を優先します。1 本指が外周で円を描いていると確定すると、その接触シーケンス中の転送を**抑止**します（カーソル凍結）。同時に、隣接サンプル間の方向ベクトル角を積分し、±π を超えるたびに仮想ホイールから 1 ノッチを発行します。全ての指が離れると位置情報を除去し、捕捉した MT スロットを解放して接触サマリーキーをクリアし、合成的なカーソルジャンプや古い接触を残さず libinput にジェスチャ終了を伝えます。
